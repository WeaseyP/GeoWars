...
float stars(vec2 uv_star, float density_threshold, float brightness_pow, float star_radius, float aa_width) {
    ...
    for (int j = -1; j <= 1; j++) { for (int i = -1; i <= 1; i++) {
            ...
            if (star_exists_val > density_threshold) {
                ...
                twinkle = min_twinkle_bright + (1.0 - min_twinkle_bright) * twinkle;                  
...
void main() {              
    if (bg_option == 0) {
        ...
        float star_rad       = 0.03;
        float star_aa        = 0.06;
        float min_twinkle_bright = 0.6;                                     
        float overall_star_brightness_multiplier = 1.8;                       
        ...
        if (star_mask > 0.001) {
            ...
            if (star_exists_val > density_thresh) {
                ...
                float twinkle_speed_variance = 1.5;                      
                float twinkle_base_speed = 0.5;                  
