       ...
       shared.state.player_hp > 0 {
        ...
        if game_is_now_won {
            ...
            // To stop further stage loads if game is won:
             shared.state.progression.active_stage.all_enemies_for_stage_spawned = true; // Keep this true
             shared.state.progression.enemies_defeated_in_current_stage = shared.state.progression.total_enemies_defined_for_current_stage + 1; // Ensure condition above doesn't re-trigger
        } else {
...
handle_enemy_spawning :: proc(dt: f32, aspect_f: f32) {
    ...
    if !shared.state.progression.active_stage.all_enemies_for_stage_spawned && shared.state.player_hp > 0 { // Only spawn if player is alive
                ...
                for &spawn_state, idx in &shared.state.progression.active_stage.enemy_spawn_states {
                    ...
                    if spawn_state.spawn_timer <= 0.0 {
                        if spawn_state.config_index < len(current_stage_def.enemy_configs) {
                            ...
                            enemy.spawn_enemy(current_ortho_width_for_spawn_f, ORTHO_HEIGHT, shared.state.player_pos, config.enemy_type);
...
load_and_initialize_stage_progression :: proc(level_idx: int, stage_idx: int) {
    ...
    if level_idx >= len(shared.game_levels) {
        ...
        shared.state.progression.active_stage.all_enemies_for_stage_spawned = true; // Stop spawning
        return;
    }
    ...
    clear(&shared.state.progression.active_stage.enemy_spawn_states); // Clear previous states
    
    total_enemies_count_for_stage: int = 0;
    if len(current_stage_def.enemy_configs) > 0 {
        // Reserve if desired, but append will grow it: reserve(&shared.state.progression.active_stage.enemy_spawn_states, len(current_stage_def.enemy_configs));
